...
//competitionId pitaj
router.post("/score_gameUpdate", authRequired, function (req, res, next) {
    console.log("updating");
    const score_game = req.body.score_game;

    const stmt = db.prepare("UPDATE signed_up SET score_game = ? WHERE userid = ?");
    const updateResult = stmt.run(score_game, req.user.sub);
